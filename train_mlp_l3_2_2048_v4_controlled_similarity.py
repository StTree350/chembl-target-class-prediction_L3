...
VERSION = "v4_controlled_similarity"
...
CLUSTER_STATS_PATH = REPORTS_METRICS_DIR / f"{RUN_NAME}_cluster_stats.csv"
SPLIT_SUMMARY_PATH = REPORTS_METRICS_DIR / f"{RUN_NAME}_split_summary.csv"
SIMILARITY_SUMMARY_PATH = REPORTS_METRICS_DIR / f"{RUN_NAME}_similarity_summary.csv"
SIMILARITY_BY_CLASS_PATH = REPORTS_METRICS_DIR / f"{RUN_NAME}_similarity_by_class.csv"
SIMILARITY_BINS_PATH = REPORTS_METRICS_DIR / f"{RUN_NAME}_similarity_bins.csv"

CLUSTER_IDS_CACHE_PATH = PROCESSED_DIR / "target_class_l3_2_2048_kmeans_500_cluster_ids.npy"
...
# Softer controlled similarity targets than the previous v4.
VALIDATION_MIN_SIMILARITY = 0.50
VALIDATION_MAX_SIMILARITY = 0.75
VALIDATION_TARGET_SIMILARITY = 0.62

TEST_MIN_SIMILARITY = 0.40
TEST_MAX_SIMILARITY = 0.60
TEST_TARGET_SIMILARITY = 0.50

TANIMOTO_BATCH_SIZE = 256
...
def compute_chemical_clusters(x: np.ndarray) -> np.ndarray:
    if CLUSTER_IDS_CACHE_PATH.exists():
        print("Loading cached MiniBatchKMeans cluster IDs...")
        print(f"Cache path: {CLUSTER_IDS_CACHE_PATH}")

        cluster_ids = np.load(CLUSTER_IDS_CACHE_PATH).astype(np.int64)

        if len(cluster_ids) != len(x):
            raise ValueError(
                f"Cached cluster ID length mismatch: "
                f"cluster_ids={len(cluster_ids)}, X={len(x)}"
            )

        return cluster_ids

    print("Computing MiniBatchKMeans chemical clusters...")
    print("=" * 80)
    print(f"n_clusters: {N_CLUSTERS}")
    print(f"batch_size: {KMEANS_BATCH_SIZE}")
    print(f"max_iter: {KMEANS_MAX_ITER}")
    ...
    cluster_ids = kmeans.fit_predict(x_float).astype(np.int64)

    np.save(CLUSTER_IDS_CACHE_PATH, cluster_ids)
    ...
    print(f"Cluster IDs cached to: {CLUSTER_IDS_CACHE_PATH}")
    print()
...
def build_cluster_groups(
    ...
    for cluster_id in np.unique(cluster_ids):
        indices = np.where(cluster_ids == cluster_id)[0].astype(np.int64)
        labels = y[indices]
        ...
        row = {
            ...
            "n_classes_in_cluster": int(np.count_nonzero(class_counts)),
        }

        for class_id in range(n_classes):
            row[f"class_{class_id}_count"] = int(class_counts[class_id])

        rows.append(row)

    cluster_groups = pd.DataFrame(rows)

    rng = np.random.default_rng(RANDOM_STATE)
    cluster_groups["random_tiebreaker"] = rng.random(len(cluster_groups))

    cluster_groups = cluster_groups.sort_values(
        by=["size", "n_classes_in_cluster", "random_tiebreaker"],
...
def assign_cluster_groups_to_provisional_splits(
    ...
    for _, row in cluster_groups.iterrows():
        ...
        for candidate_split in split_names:
            hypothetical_total = {
                split: current_total_by_split[split]
                for split in split_names
            }

            hypothetical_class_counts = {
                split: current_class_counts_by_split[split].copy()
                for split in split_names
            }

            hypothetical_total[candidate_split] += group_size
            hypothetical_class_counts[candidate_split] += group_class_counts

            size_score = 0.0

            for split in split_names:
                target_total = target_total_by_split[split]
                size_score += (
                    (hypothetical_total[split] - target_total)
            ...
            for split in split_names:
                target_class_counts = target_class_counts_by_split[split]
                class_score += np.mean(
                    (
                        (hypothetical_class_counts[split] - target_class_counts)
            ...
            for split in split_names:
                target_total = target_total_by_split[split]
                overflow = max(0.0, hypothetical_total[split] - target_total)
                overflow_score += (overflow / max(target_total, 1.0)) ** 2

            score = 5.0 * size_score + 1.0 * class_score + 2.0 * overflow_score
        ...
        assignment_rows.append(
            {
                ...
                "provisional_split": best_split,
                "provisional_assignment_score": float(best_score),
            }
        )
...
def compute_nearest_train_similarity_for_indices(
    x: np.ndarray,
    ...
    print()
    print(f"Computing nearest-train Tanimoto similarity for: {split_name}")
    print("=" * 80)
    print(f"Train molecules: {len(train_indices)}")
    print(f"Query molecules: {len(query_indices)}")
    print(f"Batch size: {TANIMOTO_BATCH_SIZE}")
    print("=" * 80)
    ...
    for start in range(0, len(query_indices), TANIMOTO_BATCH_SIZE):
        end = min(start + TANIMOTO_BATCH_SIZE, len(query_indices))
        ...
        nearest_positions = np.argmax(similarities, axis=1)
        nearest_similarities = similarities[
            np.arange(len(batch_query_indices)),
            nearest_positions,
        ]

        for local_index, row_index in enumerate(batch_query_indices):
            nearest_train_index = int(train_indices[nearest_positions[local_index]])
            ...
            rows.append(
                {
                    ...
                    "nearest_train_similarity": float(
                        nearest_similarities[local_index]
                    ),
                    "nearest_train_row_index": nearest_train_index,
                }
            )
        ...
        print(
            f"{split_name}: processed {processed}/{len(query_indices)} "
...
def similarity_penalty_for_split(median_similarity: float, candidate_split: str) -> float:
    if candidate_split == "validation":
        min_similarity = VALIDATION_MIN_SIMILARITY
        max_similarity = VALIDATION_MAX_SIMILARITY
        target_similarity = VALIDATION_TARGET_SIMILARITY
    else:
        min_similarity = TEST_MIN_SIMILARITY
        max_similarity = TEST_MAX_SIMILARITY
        target_similarity = TEST_TARGET_SIMILARITY

    if min_similarity <= median_similarity <= max_similarity:
        return abs(median_similarity - target_similarity)

    if median_similarity < min_similarity:
        return 1.0 + 3.0 * (min_similarity - median_similarity)

    return 1.0 + 3.0 * (median_similarity - max_similarity)


def controlled_holdout_assignment(
    holdout_cluster_stats: pd.DataFrame,
    y: np.ndarray,
    n_classes: int,
) -> pd.DataFrame:
    ...
    target_total_by_split = {
        "validation": VALIDATION_SIZE * len(y),
        "test": TEST_SIZE * len(y),
    }

    total_class_counts = np.bincount(y, minlength=n_classes).astype(np.float64)

    target_class_counts_by_split = {
        "validation": VALIDATION_SIZE * total_class_counts,
        "test": TEST_SIZE * total_class_counts,
    }
    ...
    current_class_counts_by_split = {
        "validation": np.zeros(n_classes, dtype=np.float64),
        "test": np.zeros(n_classes, dtype=np.float64),
    }

    class_columns = [f"class_{class_id}_count" for class_id in range(n_classes)]

    cluster_order = holdout_cluster_stats.sort_values(
        by=["size", "nearest_train_similarity_median", "cluster_id"],
        ascending=[False, False, True],
    ).reset_index(drop=True)
    ...
    for _, row in cluster_order.iterrows():
        group_size = float(row["size"])
        group_class_counts = row[class_columns].to_numpy(dtype=np.float64)
        median_similarity = float(row["nearest_train_similarity_median"])

        best_split = None
        best_score = np.inf
        best_similarity_penalty = None
        ...
        for candidate_split in split_names:
            ...
            similarity_penalty = similarity_penalty_for_split(
            ...
            score = (
                ...
                + 3.0 * class_score
                + 2.0 * overflow_score
                + 1.5 * similarity_penalty
            )

            if score < best_score:
                best_score = score
                best_split = candidate_split
                best_similarity_penalty = similarity_penalty
        ...
        assignment_rows.append(
            {
                "cluster_id": int(row["cluster_id"]),
                "split": best_split,
                "controlled_assignment_score": float(best_score),
                "similarity_penalty": float(best_similarity_penalty),
            }
        )
...
def make_controlled_similarity_splits(
    x: np.ndarray,
    y: np.ndarray,
    n_classes: int,
) -> tuple[np.ndarray, np.ndarray, np.ndarray, pd.DataFrame, pd.DataFrame]:
    cluster_ids = compute_chemical_clusters(x)
    ...
    print(f"Largest cluster size: {cluster_groups['size'].max()}")
    print(f"Median cluster size: {cluster_groups['size'].median():.1f}")
    print(f"Mean cluster size: {cluster_groups['size'].mean():.1f}")
    print("=" * 80)
    print()

    provisional_assignments = assign_cluster_groups_to_provisional_splits(
        cluster_groups=cluster_groups,
        y=y,
        n_classes=n_classes,
    )

    cluster_groups = cluster_groups.merge(
        provisional_assignments,
        on="cluster_id",
        how="left",
    )

    train_cluster_ids = cluster_groups.loc[
        cluster_groups["provisional_split"] == "train",
        "cluster_id",
    ].tolist()

    train_indices = np.concatenate(
        cluster_groups.loc[
            cluster_groups["cluster_id"].isin(train_cluster_ids),
            "indices",
        ].tolist()
    ).astype(np.int64)

    holdout_cluster_groups = cluster_groups[
        ~cluster_groups["cluster_id"].isin(train_cluster_ids)
    ].copy()

    holdout_indices = np.concatenate(
        holdout_cluster_groups["indices"].tolist()
    ).astype(np.int64)

    holdout_similarity = compute_nearest_train_similarity_for_indices(
        x=x,
        train_indices=train_indices,
        query_indices=holdout_indices,
        split_name="holdout",
    )

    row_to_cluster = pd.DataFrame(
        {
            "row_index": np.arange(len(cluster_ids), dtype=np.int64),
            "cluster_id": cluster_ids.astype(np.int64),
        }
    )

    holdout_similarity = holdout_similarity.merge(
        row_to_cluster,
        on="row_index",
        how="left",
    )
    ...
    cluster_similarity_stats = (
        ...
        .agg(
            ...
            nearest_train_similarity_min=("nearest_train_similarity", "min"),
            nearest_train_similarity_q25=("nearest_train_similarity", lambda values: values.quantile(0.25)),
            nearest_train_similarity_q75=("nearest_train_similarity", lambda values: values.quantile(0.75)),
            nearest_train_similarity_max=("nearest_train_similarity", "max"),
        )
        .reset_index()
    )

    holdout_cluster_stats = holdout_cluster_groups.merge(
        cluster_similarity_stats,
        on="cluster_id",
        how="left",
    )

    controlled_assignments = controlled_holdout_assignment(
        holdout_cluster_stats=holdout_cluster_stats,
        y=y,
        n_classes=n_classes,
    )

    holdout_cluster_stats = holdout_cluster_stats.merge(
        controlled_assignments,
        on="cluster_id",
        how="left",
    )

    validation_cluster_ids = holdout_cluster_stats.loc[
        holdout_cluster_stats["split"] == "validation",
        "cluster_id",
    ].tolist()

    test_cluster_ids = holdout_cluster_stats.loc[
        holdout_cluster_stats["split"] == "test",
        "cluster_id",
    ].tolist()

    validation_indices = np.concatenate(
        holdout_cluster_stats.loc[
            holdout_cluster_stats["cluster_id"].isin(validation_cluster_ids),
            "indices",
        ].tolist()
    ).astype(np.int64)
    ...
    test_indices = np.concatenate(
        holdout_cluster_stats.loc[
            holdout_cluster_stats["cluster_id"].isin(test_cluster_ids),
            "indices",
        ].tolist()
    ).astype(np.int64)

    split_similarity_data = holdout_similarity.merge(
        controlled_assignments,
        on="cluster_id",
        how="left",
    )

    if "split" not in split_similarity_data.columns:
        raise RuntimeError(
            "Controlled split assignment failed: column 'split' was not created."
        )

    train_cluster_export = cluster_groups[
        cluster_groups["cluster_id"].isin(train_cluster_ids)
    ].copy()

    train_cluster_export["split"] = "train"
    train_cluster_export["nearest_train_similarity_mean"] = np.nan
    train_cluster_export["nearest_train_similarity_median"] = np.nan
    train_cluster_export["nearest_train_similarity_min"] = np.nan
    train_cluster_export["nearest_train_similarity_q25"] = np.nan
    train_cluster_export["nearest_train_similarity_q75"] = np.nan
    train_cluster_export["nearest_train_similarity_max"] = np.nan
    train_cluster_export["controlled_assignment_score"] = np.nan
    train_cluster_export["similarity_penalty"] = np.nan

    cluster_stats_export = pd.concat(
        [
            train_cluster_export,
            holdout_cluster_stats,
        ],
        axis=0,
        ignore_index=True,
    )

    cluster_stats_export = cluster_stats_export.drop(
        columns=["indices", "random_tiebreaker"],
        errors="ignore",
    ).sort_values(
        by=["split", "cluster_id"],
        ascending=[True, True],
    ...
    return (
        ...
        validation_indices,
        test_indices,
        cluster_stats_export,
        split_similarity_data,
    )


def make_similarity_summaries(
    split_similarity_data: pd.DataFrame,
    y: np.ndarray,
    class_names: list[str],
) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    enriched = split_similarity_data.copy()

    if "split" not in enriched.columns:
        raise ValueError(
            f"Column 'split' not found in split_similarity_data. "
            f"Available columns: {list(enriched.columns)}"
        )

    enriched["true_label_id"] = y[enriched["row_index"].to_numpy(dtype=np.int64)]

    enriched["true_target_class_l3"] = [
        class_names[label]
        for label in enriched["true_label_id"].astype(int).tolist()
    ]

    summary_rows = []

    for split_name, split_data in enriched.groupby("split"):
        row = {
            "split": split_name,
            "samples": len(split_data),
            "nearest_train_similarity_mean": split_data["nearest_train_similarity"].mean(),
            "nearest_train_similarity_std": split_data["nearest_train_similarity"].std(),
            "nearest_train_similarity_min": split_data["nearest_train_similarity"].min(),
            "nearest_train_similarity_q01": split_data["nearest_train_similarity"].quantile(0.01),
            "nearest_train_similarity_q05": split_data["nearest_train_similarity"].quantile(0.05),
            "nearest_train_similarity_q10": split_data["nearest_train_similarity"].quantile(0.10),
            "nearest_train_similarity_q25": split_data["nearest_train_similarity"].quantile(0.25),
            "nearest_train_similarity_median": split_data["nearest_train_similarity"].median(),
            "nearest_train_similarity_q75": split_data["nearest_train_similarity"].quantile(0.75),
            "nearest_train_similarity_q90": split_data["nearest_train_similarity"].quantile(0.90),
            "nearest_train_similarity_q95": split_data["nearest_train_similarity"].quantile(0.95),
            "nearest_train_similarity_q99": split_data["nearest_train_similarity"].quantile(0.99),
            "nearest_train_similarity_max": split_data["nearest_train_similarity"].max(),
        }
    ...
    similarity_summary = pd.DataFrame(summary_rows)
    ...
    enriched["nearest_train_similarity_bin"] = pd.cut(
        enriched["nearest_train_similarity"],
        bins=bins,
        labels=labels,
        include_lowest=True,
        right=False,
    ...
    class_summary.to_csv(
        SIMILARITY_BY_CLASS_PATH,
        index=False,
        encoding="utf-8",
    )

    bin_summary.to_csv(
        SIMILARITY_BINS_PATH,
        index=False,
        encoding="utf-8",
    )

    return similarity_summary, class_summary, bin_summary
...
def save_split_assignments(
    ...
    split_data = split_data.reset_index().rename(columns={"index": "row_index"})
    split_data["cluster_id"] = cluster_ids

    similarity_columns = split_similarity_data[
        [
            "row_index",
            "nearest_train_similarity",
            "nearest_train_row_index",
        ]
    ].copy()
...
def save_config(
    ...
    config = {
        ...
        "split": {
            "split_type": "controlled_similarity_split_soft_v4_overwrite",
            ...
            "validation_min_similarity": VALIDATION_MIN_SIMILARITY,
            "validation_max_similarity": VALIDATION_MAX_SIMILARITY,
            "validation_target_similarity": VALIDATION_TARGET_SIMILARITY,
            "test_min_similarity": TEST_MIN_SIMILARITY,
            "test_max_similarity": TEST_MAX_SIMILARITY,
            "test_target_similarity": TEST_TARGET_SIMILARITY,
            "random_state": RANDOM_STATE,
        ...
        "output_files": {
            ...
            "cluster_stats_path": str(CLUSTER_STATS_PATH),
...
def main() -> None:
    ...
    print(f"Split type: controlled similarity split, softer v4 overwrite")
    ...
    print()
    print("Controlled split targets")
    print("-" * 80)
    print(
        f"Validation target: nearest-train Tanimoto "
        f"{VALIDATION_MIN_SIMILARITY}-{VALIDATION_MAX_SIMILARITY}"
    )
    print(
        f"Test target: nearest-train Tanimoto "
        f"{TEST_MIN_SIMILARITY}-{TEST_MAX_SIMILARITY}"
    )
    print("=" * 80)
    print()
    ...
    (
        ...
        cluster_stats,
        split_similarity_data,
    ) = make_controlled_similarity_splits(
        x=x,
        y=y,
        n_classes=n_classes,
    )

    similarity_summary, _, _ = make_similarity_summaries(
        split_similarity_data=split_similarity_data,
        y=y,
        class_names=class_names,
    )
